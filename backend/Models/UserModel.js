...
function getPassword(email) {
  return new Promise((resolve, reject) => {
    ...
    const query =
      'SELECT u.mot_de_passe,r.designation FROM utilisateur u ,role r WHERE email= ? and u.id_role=r.id_role ';
    const values = [email];
...
function updateInformations(
  ...
  return new Promise((resolve, reject) => {
    ...
    ${
      (type && structureId) ||
      ((nom || prenom || date_naissance) && structureId)
        ? ', '
        : ''
    }
    ...
    ${
      (structureId && role) ||
      ((nom || prenom || date_naissance || type) && role)
        ? ', '
        : ''
    }
...
function createConsommateur(
  ...
  return new Promise((resolve, reject) => {
    ...
    const query =
      'insert into consommateur (email,nom,prenom,date_naissance,id_structure,type) values (?,?,?,?,?,?)';
...
function getStructures() {
  return new Promise((resolve, reject) => {
    ...
    const query =
      'select s.designation,u.nom,u.prenom from structure s,utilisateur u where s.id_resp=u.email ';
...
function addResponsable(email, nom, prenom, date_naissance) {
  return new Promise((resolve, reject) => {
    ...
    const query =
      'insert into responsable (email,nom,prenom,date_naissance) values(?,?,?,?)';
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.connect(err => {
      ...
      connection.query(query, (error, results, fields) => {
        ...
        const groupedResults = Object.entries(groupedByRole).map(
          ([role, droits]) => ({ role, droits })
        );
...
function insertRoleDroit(id_role, permissions) {
  return new Promise((resolve, reject) => {
    ...
    query += `Select ${id_role},id_droit from droit_acces where designation='${
