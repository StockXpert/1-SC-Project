...
function getRefs(produit)
{
  return new Promise((resolve, reject) => {
    const connection = mysql.createConnection(connectionConfig);
    const query = `select p.designation as produit,r.designation ,r.num_inventaire,r.date_inventaire as reference from reference r ,produit p where p.id_produit=r.id_produit 
