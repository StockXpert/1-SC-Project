...
function insertLink(link, link2, n_commande) {
  return new Promise((resolve, reject) => {
    const connection = mysql.createConnection(connectionConfig);
    const query =
      'update bon_de_commande set link=?,excel_link=? where num_commande=?';
...
function insertReceptionLink(link, link2, numReception) {
  return new Promise((resolve, reject) => {
    const connection = mysql.createConnection(connectionConfig);
    const query =
      'update bon_de_reception set link=?,excel_link=? where num_bon=?';
          ...
          (produit, callback) => {
            ...
            connection.query(
              ...
              (err, rows) => {
                ...
                connection.query(
                  'INSERT INTO commande (id_commande, id_produit, quantite, prix_unitaire) VALUES (?, ?, ?, ?)',
                  [
                    n_commande,
                    id_produit,
                    produit.quantite,
          ...
          (produit, callback) => {
            ...
            connection.query(
              ...
              (err, rows) => {
                ...
                // Insérer les données dans ma_table avec l'ID produit récupéré
                connection.query(
                  'INSERT INTO livre (num_bon, id_produit, quantite) VALUES (?, ?, ?)',
                  [numReception, id_produit, produit.quantite],
                  (err, result) => {
                    if (err) {
                      return callback(err);
                    }
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.connect(err => {
      ...
      connection.query(query, values, (error, results, fields) => {
        ...
        if (results.length === 0) reject('erreur');
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.connect(err => {
      ...
      connection.query(query, values, (error, results, fields) => {
        ...
        console.log(results);
        if (results.length === 0) resolve('valid');
...
function updateBonCommande(fournisseur, objet, date, numCommande) {
  return new Promise((resolve, reject) => {
    const connection = mysql.createConnection(connectionConfig);
    const query = `update bon_de_commande set ${
      fournisseur
        ? 'id_fournisseur=(select id_fournisseur from fournisseur where raison_sociale=?)'
        : ''
    } 
          ${objet && fournisseur ? ',' : ''} ${
      objet
        ? 'id_article=(select id_article from article where designation=?)'
        : ''
    } 
          ${(objet && date) || (fournisseur && date) ? ',' : ''} ${
      date ? 'date_commande=?' : ''
    } where num_commande=?`;
    const values = [];
          ...
          (produit, callback) => {
            ...
            connection.query(
              ...
              (err, rows) => {
                ...
                connection.query(
                  'DELETE FROM commande WHERE id_commande = ? AND id_produit = ?',
                  [n_commande, id_produit],
                  (err, result) => {
                    if (err) {
                      return callback(err);
                    }
                    console.log(
                      "Produit supprimé avec succès de ma_table avec l'ID produit : ",
                      id_produit
                    );
                    callback();
                  }
...
function updateReception(
  ...
  return new Promise((resolve, reject) => {
    const connection = mysql.createConnection(connectionConfig);
    const query = `update bon_de_reception set ${
      numLivraison ? 'num_livraison=?' : ''
    }
                  ${numLivraison && numFacture ? ',' : ''} ${
      numFacture ? 'num_facture=?' : ''
    } 
                  ${
                    (numLivraison && dateReception) ||
                    (numFacture && dateReception)
                      ? ','
                      : ''
                  }
