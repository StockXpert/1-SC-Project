...
function addChapter(req, res) {
  ...
  NomenclatureModel.insertChapter(numChap, designation)
    .then(() => {
...
function updateChapter(req, res) {
  ...
  NomenclatureModel.updateChapter(oldDesignation, newDesignation)
    .then(() => {
...
function deleteChapter(req, res) {
  ...
  NomenclatureModel.isUsedchapter(designation)
    .then(() => {
      NomenclatureModel.canDelete(designation, 'chapitre')
        .then(() => {
          NomenclatureModel.deleteChapter(designation)
            .then(() => {
...
function addArticle(req, res) {
  ...
  NomenclatureService.addArticle(chapitre, designation, numArt, tva)
    .then(response => {
...
function deleteFournisseur(raisonSocial) {
  NomenclatureModel.canDeletefourn(raisonSocial)
    .then(() => {
      NomenclatureModel.deleteFournisseur(raisonSocial)
        .then(() => {
    ...
    .catch(() => {
      res.status(500).json({ response: 'prohibited to delete fournisseur' });
    });
}
function updateArticle(req, res) {
  const { oldDesignation, newDesignation, chapitre, tva } = req.body;
  NomenclatureModel.updateArticle(oldDesignation, newDesignation, chapitre, tva)
    .then(() => {
...
function deleteArticle(req, res) {
  const { designation } = req.body;
  NomenclatureModel.isUsedArticle(designation)
    .then(() => {
      NomenclatureModel.canDelete(designation, 'article')
        .then(() => {
          NomenclatureService.deleteArticle(designation)
            .then(response => {
              res.status(200).json({ response });
            })
            .catch(response => {
...
function addProduct(req, res) {
  const { article, designation, quantite, description, seuil, consommable } =
    req.body;
  NomenclatureService.addProduct(
    article,
    designation,
    description,
    quantite,
...
function deleteProduct(req, res) {
  const { designation } = req.body;
  NomenclatureModel.isUsedProduct(designation)
    .then(() => {
      NomenclatureService.deleteProduct(designation)
        .then(response => {
          res.status(200).json({ response });
        })
        .catch(response => {
...
function addFournisseur(req, res) {
  const { raisonSociale, adresse, tel, fax, numRegistre, ribRip, nif, nis } =
    req.body;
  NomenclatureModel.insertFournisseur(
    raisonSociale,
    adresse,
    tel,
    fax,
...
function deleteFournisseur(req, res) {
  ...
  NomenclatureModel.isUsedFournisseur(raisonSociale)
    .then(() => {
      NomenclatureModel.canDeletefourn(raisonSociale)
        .then(() => {
          NomenclatureModel.deleteFournisseur(raisonSociale)
            .then(() => {
              res.status(200).json({ response: 'fournisseur deleted' });
            })
            .catch(() => {
...
function updateFournisseur(req, res) {
  ...
  NomenclatureModel.updateFournisseur(
    adresse,
    telephone,
    fax,
    numRegistre,
...
function updateRaisonSociale(req, res) {
  ...
  NomenclatureModel.updateRaisonSociale(newRaisonSociale, oldRaisonSociale)
    .then(() => {
      res.status(200).json({ response: 'fournisseur updated' });
    })
    .catch(() => {
...
function showRefs(req, res) {
  ...
  NomenclatureModel.getRefs(produit)
    .then(refs => {
