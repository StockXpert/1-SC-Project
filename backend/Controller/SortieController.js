...
function demandeFourniture(req, res) {
  ...
  const { email } = req;
  SortieModel.addFourniture(email, dateDemande, exterieur)
    .then(numDemande => {
      SortieModel.insertFournir(numDemande, produits)
        .then(() => {
  ...
  SortieModel.getDemandeProducts(numDemande)
    .then(produits => {
      SortieService.subtituteQuantite(produits)
        .then(() => {
          SortieModel.changeDemandeStatNotif(numDemande, 'servie', 'cons_notif')
            .then(() => {
              SortieModel.isExterior(numDemande)
                .then(exterior => {
                  if (exterior) {
                    ...
                    SortieModel.getDemandeurStructure(numDemande)
                      .then(structure => {
                        SortieService.genererBonSortie(
                          numDemande,
                          dateSortie,
                          produits,
...
function deleteFourniture(req, res) {
  ...
  SortieModel.canDeleteFourniture(numDemande)
    .then(() => {
      SortieModel.deleteFourniture(numDemande)
        .then(() => {
...
function showAllDemandes(req, res) {
  ...
  SortieModel.getAllDemandes(etat, statement, email, role)
    .then(demandes => {
...
function showNewDemandes(req, res) {
  ...
  SortieModel.getNewDemandes(
    ...
    etat,
    email,
    role === 'consommateur' ? 'cons_notif' : 'other_notif'
  )
    .then(demandes => {
      res.status(200).json({ response: demandes });
    })
...
function updateConsDemande(req, res) {
  ...
  SortieModel.canDeleteFourniture(numDemande)
    .then(() => {
      SortieModel.insertFournir(numDemande, addedProducts)
        .then(() => {
          SortieModel.deleteProductsFournir(numDemande, deletedProducts)
            .then(() => {
...
function updateMagApp(req, res) {
  ...
  SortieService.canUpdate(numDemande, role)
    .then(() => {
      SortieModel.updateLivredQuantite(numDemande, produits)
        .then(() => {
          res.status(200).json({ response: 'updated' });
...
function showDemande(req, res) {
  ...
  SortieModel.getDemande(numDemande, role, quantiteType)
    .then(demande => {
