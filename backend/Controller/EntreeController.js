...
function updateQuantite(req,res)
{
   ...
   const {numCommande,numFacture,numLivraison,dateReception,produits}=req.body;
   //const produits=JSON.parse(req.body.produits);
   console.log(produits);
   console.log(req.files)
   const factureLink = null//'Facture/'+(req.files['facture']?req.files['facture'][0].filename:null);
   const bonLivraisonLink = null;//'bonLivraison/'+req.files['bonLivraison'][0].filename
  ...
  EntreeModel.getBonReception(numCommande).then((response)=>{
   ...
   else
   {
      EntreeService.changeBonCommande(objet,fournisseur,deletedProducts,addedProducts,date,numCommande).then(()=>{
         EntreeModel.getBonCommande(numCommande).then((commande)=>{
          EntreeModel.getBonCommandeFournisseur(numCommande).then((raisonSociale)=>{
            EntreeModel.getCommandeProducts(numCommande).then((products)=>{
               getArticleIdTva(commande.objet).then((article)=>{
                  console.log({raisonSociale})
                  EntreeService.genererBondeCommande(numCommande,products,raisonSociale,commande.objet,commande.type,'1KEtktsb0n8ZspuxitPk-8kX1S5OJ6AcJmnlbywh-s98',article.tva,commande.date_commande)
