...
async function login(req,res)
{
    ...
    userService.verifyUser(email).then((response)=>
    {
       if(response==="email available")
        ...
        bcrypt.compare(password,response.password).then((result)=>
        {
         ...
         if(result)
         {   
         const token=jwt.sign({email:email,role:response.role},SecretKey,{expiresIn:'30m'});
         res.status(200).json({response:"succuss of login",jwt:token})}
...
async function register(req,res)
{
    ...
    userService.createUser(email,role,encry_pswrd).then((response)=>{
        if((role!="Consommateur"&&role!="RD"&&role!="DG")||response!="utilisateur insere")
           return res.json({response})
        if(role=="RD"||role=="DG")
        {
            const {nom,prenom,email,date_naissance}=req.body;
            userService.createResponsable(nom,prenom,email,date_naissance).then((response)=>{
           ...
           userService.createConsommateur(nom,prenom,date_naissance,structure,email,type)
           .then(()=>{
...
{
    userModel.getUser(req.email).then((results)=>
    {
        res.status(200).json({response:results});
    }).catch((error)=>
    {
...
function forgotPassword(req,res)
{
    ...
    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          res.status(200).send('Un e-mail de réinitialisation de mot de passe a été envoyé.');
        }
      });
}
    ...
    if(passwordReset[email]===code)
       {
        const encry_pswrd=await bcrypt.hash(newPassword,10);
         userModel.changePassword(email,encry_pswrd).then(()=>{
...
function updateConsumerInformations(req,res)
{
    ...
    userService.updateConsumerInfos(email,nom,prenom,date_naissance,type,structure).then(()=>{
...
function addStructure(req,res)
{
  ...
  userModel.addStructure(designation,email).then(()=>{
...
{
    userModel.afficherConsommateurs().then((response)=>{
        res.status(200).json({response});
    }).catch(()=>{
        res.status(500).json({response:"internal error"})
    })
...
function rattacher(req,res)
{
    ...
    userService.rattacher(email,structure).then((response)=>{
        res.status(200).json({response});
    }).catch((response)=>{
...
function responsable(req,res){
    const {structure,email}=req.body;
    userService.responsable(email,structure).then((response)=>{
        res.status(200).json({response});
    }).catch((response)=>{
        res.status(400).json({response});
    });
...
{
    userModel.getStructures().then((response)=>{
        res.status(200).json({response})
    }).catch(()=>{
        res.status(500).json({response:"internal error"});
    })
...
{
    const {nom,prenom,email,date_naissance}=req.body
   userModel.updateInformations(email,nom,prenom,date_naissance,null,null,"responsable").then(()=>{
    res.status(200).json({response:"infomations updated"})
   }).catch({response:"internal error"})
}
function showResp(req,res)
{
    userModel.showResp().then((response)=>{
        res.status(200).json({response})
    }).catch(()=>{
        res.status(200).json({response:"internal error"});
    })
}
module.exports={login,register,showUsers,showUser,forgotPassword,changePasswordAuth,
                changePasswordMail,updateConsumerInformations,deleteUser,
                addStructure,afficherConsommateurs,rattacher,responsable,showStructure,
                updateRespInformations,showResp};